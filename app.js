...
function applyStructure(type){
  if(!STRUCTURES[type]) return toast('Unknown structure');
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    cy.resize();
    try{runStructure(type)}catch(error){console.error(error);posManual(type);readableFit(0)}
    snapshot();
    updateEmpty();
