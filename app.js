...
const cy=cytoscape({container:$('#cy'),elements:[],minZoom:.12,maxZoom:4,wheelSensitivity:.18,boxSelectionEnabled:true,selectionType:'additive',style:[
...
function restore(raw){state.restoring=true;try{const p=typeof raw==='string'?JSON.parse(raw):raw;cy.elements().remove();cy.add(p.elements||[]);refreshLabels();state.structure=p.structure||'mindmap';setStructureUI();cy.pan(p.pan||{x:innerWidth/2,y:innerHeight/2});cy.zoom(p.zoom||1);cy.nodes().forEach(n=>n.data('locked')?n.lock():n.unlock())}finally{state.restoring=false;updateEmpty();updateInspector()}}
...
function runStructure(type){state.structure=type;setStructureUI();const animate=$('#animateLayout').checked;const done=()=>{setTimeout(snapshot,animate?650:50)};if(['timeline','matrix','gantt','fishbone','bubble'].includes(type)){posManual(type);cy.animate({fit:{eles:cy.elements(),padding:100},duration:animate?450:0});done();return}let cfg={fit:true,padding:100,animate,animationDuration:520};if(type==='mindmap')cfg={...cfg,name:'breadthfirst',directed:true,roots:roots(),spacingFactor:1.45};if(type==='flowchart')cfg={...cfg,name:'breadthfirst',directed:true,roots:roots(),spacingFactor:1.25};if(type==='orgchart'||type==='tree')cfg={...cfg,name:'breadthfirst',directed:true,roots:roots(),spacingFactor:1.4};if(type==='concept')cfg={...cfg,name:'cose',idealEdgeLength:180,nodeRepulsion:900000,gravity:.13,numIter:900};cy.layout(cfg).run();done()}
